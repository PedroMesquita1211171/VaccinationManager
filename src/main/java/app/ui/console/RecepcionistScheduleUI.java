...
public class RecepcionistScheduleUI implements Runnable{
    ...
    public void run() {
        try{
            ...
            SNSUserDTO snsu = controller.getSNSUser(Utils.readLineFromConsole("Insert SNSUser Number:"));
    ...
    private TimeSlot askTimeSlot(VaccinationCenterDTO vc, Data date) {
        ...
        timeSlotList.removeIf(t -> !controller.slotAvailable(date, t, vc));
    ...
    private VaccineDTO askVaccine() {
        List<VaccineDTO> vaccineList = controller.getVaccines();
    ...
    private VaccinationCenterDTO askVaccinationCenter() {
        List<VaccinationCenterDTO> vcList = controller.getVaccinationCenters();
